...
function _daed_fix() { 
    ...
    # DAED_REPO="https://github.com/daeuniverse/daed.git"
    DAED_REPO=$1
    APP=$2
    CUSTOM_COMMIT=$3
    ...
    # 获取最新 commit
    COMMIT=$(git ls-remote $DAED_REPO HEAD | awk '{print $1}')

    if [ -z "$COMMIT" ]; then
      echo "Failed to fetch daed commit"
      exit 1
    fi


    # 查找所有 daed Makefile
    file="$BASE_PATH/package/$APP/Makefile"

    if [ -f "$file" ]; then
      # 强制使用 git 源
      sed -i 's#PKG_SOURCE_PROTO:=.*#PKG_SOURCE_PROTO:=git#g' "$file"

      # 修正 URL
      sed -i 's#PKG_SOURCE_URL:=.*#PKG_SOURCE_URL:='"$DAED_REPO"'#g' "$file"
      
      if [ ! -z "$CUSTOM_COMMIT" ]; then
        COMMIT=$CUSTOM_COMMIT
      fi
      echo "Using daed commit: $COMMIT"

      # 写入 commit
      if grep -q "PKG_SOURCE_VERSION" "$file"; then
        sed -i 's/PKG_SOURCE_VERSION:=.*/PKG_SOURCE_VERSION:='"$COMMIT"'/g' "$file"
      else
        echo "PKG_SOURCE_VERSION:=$COMMIT" >> "$file"
      fi

      # 避免奇怪的 hash 校验问题
      sed -i 's/PKG_HASH:=.*/PKG_HASH:=skip/g' "$file" || true
    else
      echo "No Makefile found for $file, Fix Failed"
    fi
