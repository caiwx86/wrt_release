...
function add_daed() {
  ...
  if [ -f "package/daed/Makefile" ]; then
      rm -rf package/daed/Makefile && cp -r $CUSTOM_PATCH_DIR/patch/daed/Makefile package/daed/
...
function add_dae() {
  ...
  cp -rv $CUSTOM_PATCH_DIR/package/dae ./package/
  cp -rv $CUSTOM_PATCH_DIR/package/luci-app-dae ./package/
