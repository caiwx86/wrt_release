...
remove_unwanted_packages() {
    ...
    for pkg in "${packages_net[@]}"; do
        if [[ -d ./feeds/packages/net/$pkg ]]; then
            ...
            \rm -rf ./feeds/packages/admin/$pkg
        fi
    done
...
install_small8() {
    ./scripts/feeds install -p small8 -f xray-core xray-plugin dns2tcp dns2socks haproxy hysteria \
        ...
        netdata luci-app-netdata lucky luci-app-lucky homebox luci-app-netspeedtest \
        luci-app-amlogic nikki luci-app-nikki tailscale luci-app-tailscale oaf open-app-filter luci-app-oaf \
        luci-theme-argon luci-theme-argon-config easytier luci-app-easytier msd_lite luci-app-msd_lite cups luci-app-cupsd
}
...
set_build_signature() {
    ...
    if [ -d "$(dirname "$file")" ] && [ -f $file ]; then
        sed -i "s/(\(luciversion || ''\))/(\1) + (' \/ build by Caiwx')/g" "$file"
    fi
}
